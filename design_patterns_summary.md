...
class BurgerBuilder {
  ...
  addCheese() { this.ingredients.push("cheese"); return this; }
  addLettuce() { this.ingredients.push("lettuce"); return this; }
  build() { return this.ingredients; }
}
...
```javascript
const car = { wheels: 4, drive() { return "vroom"; } };
...
class NewPrinter {
  ...
  printDocument() { this.oldPrinter.print(); }
}
```
...
```javascript
function coffee() { return "coffee"; }
function withMilk(c) { return () => c() + " + milk"; }
...
class Facade {
  ...
  start() { return this.computer.boot(); }
}
```
...
class Editor {
  ...
  save() { return this.content; }
  restore(memento) { this.content = memento; }
}
```
...
```javascript
class State {
  handle() { console.log("Handling state..."); }
}
```

### 🔹 Strategy
```javascript
function travel(strategy) { strategy(); }
...
### 🔹 Visitor
```javascript
function accept(visitor, node) { visitor.visit(node); }
```
